...
function kernel(ϵ::Array{Float64}, aux::Array{Any}, Σ::Array{Complex{Float64}})::Array{Complex{Float64}}
    denominator1 = @. ϵ - aux[1]
    numerator1 = @. ϵ - aux[3]
    numerator2 = @. aux[2] * Σ
    numerator = @. (numerator1 * denominator1) - numerator2
    @. (denominator1 + aux[2]) / numerator
end
...
function main_computation(E, nE::Int, p::parameters,
    ...
    while true
        for l ∈ 1:(2n+1)
            Gret += aux2[l] .* kernel(aux4[:, l], aux3, p.Σ)
        end
        Gret .*= amplitude
        ...
        loopexit = (abs.(init_nb .- local_nbar)) .< 0.0001
        count += 1
        if all(loopexit) || count > 50
            init_nb .= local_nbar
            break
        else
            init_nb .= local_nbar
    ...
    n_result[1] = init_nb[1]
    n_result[2] = init_nb[2]
