...
function init_parameters(;
    ...
    auxϵσ = zeros(Float64, 3, 3, 2)
    γ = zeros(Float64, 3, 3, 2)

    Λl = zeros(Float64, 3, 3, 2)
    Λr = zeros(Float64, 3, 3, 2)

    Γl = zeros(Float64, 3, 3, 2)
    Γr = zeros(Float64, 3, 3, 2)

    auxϵσ[1, 1, :] .+= ϵσ + 30*Bz[1]
    auxϵσ[2, 2, :] .+= ϵσ + 0.0*Bz[2]
    auxϵσ[3, 3, :] .+= ϵσ - 30*Bz[3]
...
function kernel(ϵ::Float64, aux::Array{Complex{Float64},3})::Array{Complex{Float64},3}
    aux2 = copy(aux)
    adj = similar(aux2)
    inv_ans = similar(aux2)
    ...
    # Compute determinant
    det = @. aux2[1, 1, :] * (aux2[2, 2, :] * aux2[3, 3, :] - aux2[2, 3, :] * aux2[3, 2, :]) -
             aux2[1, 2, :] * (aux2[2, 1, :] * aux2[3, 3, :] - aux2[2, 3, :] * aux2[3, 1, :]) +
             aux2[1, 3, :] * (aux2[2, 1, :] * aux2[3, 2, :] - aux2[2, 2, :] * aux2[3, 1, :])

    # Compute adjugate (cofactor matrix transposed)
    adj = similar(aux2)
    adj[1, 1, :] .= @. aux2[2, 2, :] * aux2[3, 3, :] - aux2[2, 3, :] * aux2[3, 2, :]
    adj[1, 2, :] .= @. -(aux2[1, 2, :] * aux2[3, 3, :] - aux2[1, 3, :] * aux2[3, 2, :])
    adj[1, 3, :] .= @. aux2[1, 2, :] * aux2[2, 3, :] - aux2[1, 3, :] * aux2[2, 2, :]

    adj[2, 1, :] .= @. -(aux2[2, 1, :] * aux2[3, 3, :] - aux2[2, 3, :] * aux2[3, 1, :])
    adj[2, 2, :] .= @. aux2[1, 1, :] * aux2[3, 3, :] - aux2[1, 3, :] * aux2[3, 1, :]
    adj[2, 3, :] .= @. -(aux2[1, 1, :] * aux2[2, 3, :] - aux2[1, 3, :] * aux2[2, 1, :])

    adj[3, 1, :] .= @. aux2[2, 1, :] * aux2[3, 2, :] - aux2[2, 2, :] * aux2[3, 1, :]
    adj[3, 2, :] .= @. -(aux2[1, 1, :] * aux2[3, 2, :] - aux2[1, 2, :] * aux2[3, 1, :])
    adj[3, 3, :] .= @. aux2[1, 1, :] * aux2[2, 2, :] - aux2[1, 2, :] * aux2[2, 1, :]

    # Compute inverse
    inv_ans[:, :, 1] .= adj[:, :, 1] ./ det[1]
    inv_ans[:, :, 2] .= adj[:, :, 2] ./ det[2]
